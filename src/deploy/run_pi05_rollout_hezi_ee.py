#!/usr/bin/env python3
"""Nero pi0.5 rollout 采集 —— hezi 封盖【末端位姿版】(7 段, 双臂, 20维位姿→IK→关节)。

模型: pi05_nero_hezi_closing_ee_v1 / hezi_ee_run1 / step 19999, serve 默认 8026。
...
# 对应 E2 stage34 → E3 stage56。录了同名轨迹就优先回放它, 没录则回退 goto_home.py 直线插值归位。
# ⚠ 这个常量之前漏了定义 —— goto_home_inline() 一调就 NameError, 整段过渡直接崩。
DEFAULT_TRAJ = "transition_e2_to_e3"
...
def main() -> None:
    ...
    ap.add_argument("--repo-id", default="local/nero_hezi_ee_rollout")
    ...
    if args.root is None:
        ...
        args.root = Path("<PROJECT_ROOT>/lerobot_data/Rlinf/data") / "wholeprocess" / run_id / "e2_hezi34"
        args.repo_id = f"local/wholeprocess_{run_id}_e2_hezi34"
            ...
            while True:
                ...
                while sid < len(subtasks):
                    ...
                    for i in range(n_exec):
                        ...
                        _T["录制"] = (time.monotonic() - _t0) * 1000; _t0 = time.monotonic()
                        frame_idx += 1
                        ...
                        last_cmd = target
                        _T["can发"] = (time.monotonic() - _t0) * 1000; _t0 = time.monotonic()
                        ...
                        if sw is not None:
                            try:
                                ...
                                sw = None
                        _T["切换器"] = (time.monotonic() - _t0) * 1000
                        ...
                        _T["合计"] = dt * 1000
                        _acc = locals().setdefault("_acc", {})
                        for _k, _v in _T.items():
                            _acc[_k] = _acc.get(_k, 0.0) + _v
                        _acc["_n"] = _acc.get("_n", 0) + 1
                        if _acc["_n"] % 100 == 0:      # 每 100 步报一次各段均值
                            _n = _acc["_n"]
                            print("\n  [耗时/步] " + "  ".join(
                                f"{_k}={_acc[_k]/_n:.1f}ms" for _k in
                                ("ik","can读","相机","录制","can发","切换器","合计") if _k in _acc)
                                + f"   → {1000*_n/max(_acc['合计'],1e-6):.1f}Hz", flush=True)
