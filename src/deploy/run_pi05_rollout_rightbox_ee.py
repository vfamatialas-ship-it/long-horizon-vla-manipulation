#!/usr/bin/env python3
"""Nero pi0.5 rollout 采集 —— 【右臂抓盒装箱·末端位姿版】(2 子任务), 边部署边录。

模型: config `pi05_nero_right_box_pick_ee_v1b` / rbp_ee_v1b_run1 / step 19999
  ckpt: <CKPT_ROOT_EE>/pi05_nero_right_box_pick_ee_v1b/rbp_ee_v1b_run1/19999
  这是**补训版**: v1 从 pi05_base 训 20000 步(loss 0.0841→0.0077), v1b 在 v1 的 19999
  存档上再训 20000 步(lr 重启到 5e-5), 最终 loss **0.0049**, 比 v1 又降 36%。
  数据 local/nero_right_box_pick_ee_v1(100 集 / 41034 帧 / 15Hz)。
...
⚠⚠ 这是【末端位姿版】, 不是关节版 —— 与 run_pi05_rollout_leftbox_v2.py 的根本区别:
      ...
      本脚本     : **10 维绝对末端位姿**进出(x,y,z + rot6D + 爪宽), 必须【接 IK】
                   解算成关节角才能驱动机械臂。
  骨架取自 run_pi05_rollout_trash_ee.py(同为右臂单臂 EE + IK, 已打通链路), 不抄关节版。
  IK 的两个坑沿用那边的规避:
...
═══ 单臂 ═══
只有右臂执行。左臂不参与 —— 本脚本【完全不碰左臂】, 连读都不读
(数据集 state/action 就是 10 维, 没有左臂的位置)。

═══ 相机 ═══
config 的 repack 只取两路 RGB:
  observation.images.third_view  → observation/image      (base_0_rgb)
  observation.images.right_wrist → observation/wrist_image (left_wrist_0_rgb)
  第三槽零填 + mask=False —— 左腕相机不开。

═══ 使用 ═══
0. 起服务(<SERVER>, GPU0):
     GPU=0 PORT=8026 STEP=19999 bash ~/项目/nero/deploy/ee_server/serve_rightbox_ee.sh
1. 清代理: export no_proxy=<POLICY_SERVER_IP> ; unset http_proxy https_proxy all_proxy
2. 干跑验键位:  python3 run_pi05_rollout_rightbox_ee.py --mock-robot --mock-cameras
3. 真机 dry-run: python3 run_pi05_rollout_rightbox_ee.py            (只推理只录, 不发运动)
4. 真执行:      python3 run_pi05_rollout_rightbox_ee.py --execute

键位: 空格/s = 本段成功并切下一段    f = 本段失败(选类型, 整条结束)
      g=丢弃整条   p=暂停/继续   回车/q=急停(停发+右臂下电抱闸)
相机与 CAN 按设备序列号自动检测。
"""
...
PERIOD = 1.0 / FPS
SIDE = "right"                      # 本任务只有右臂
GRIP_MAX = 0.105                    # 右爪硬件上限(数据实测最大 0.1009)
...
SUBTASKS = (
    {"task_index": 1, "zh": "从右侧区域抓一个盒子",
     "en": "Use the right arm to grasp a box from the right-side area."},
    {"task_index": 0, "zh": "把盒子放进中间打包箱的空位",
     "en": "Use the right arm to place the grasped box into an empty spot "
           "of the middle packing carton."},
)

# nero_right_box_pick_ee_v1 实测 10 维范围(2026-08-24 从 100 集 / 41034 帧 parquet 实算)。
# 维度: ee_x, ee_y, ee_z, rot6d_0..5, gripper_width
STATE_MIN = np.array([-0.1285, -0.3771, 0.7253, -0.9980, -0.6049, -0.1427,
                      -0.3827, -0.5850, -0.4600, 0.0000])
STATE_MAX = np.array([0.5248, 0.1253, 1.2199, 0.5836, 1.0000, 0.9997,
                      1.0000, 0.9998, 0.6872, 0.1009])
...
class RightArm:
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.arm = NeroArm(NeroArmConfig(
            name="right", can_channel=args.right_can, firmware=args.right_firmware,
...
def build_obs(state10: np.ndarray, frames: dict, prompt: str, preresize: bool = True) -> dict:
    """两路 RGB —— 与 config 的 repack 一一对应(third→image, right_wrist→wrist_image)。
    ...
    return {
        ...
        "observation/wrist_image": rs(frames["observation.images.right_wrist"]),
...
def create_dataset(args):
    ...
    features = {
        ...
        "joint_cmd": {"dtype": "float32", "shape": (7,),
                      "names": [f"right_j{i}" for i in range(1, 8)]},
        "ik_pos_err": dict(f32),
        "ik_ok": dict(i64),
        "active_arm": dict(s1),
        "prompt_text": dict(s1),
    ...
    return LeRobotDataset.create(repo_id=args.repo_id, fps=FPS, features=features, root=args.root,
                                 robot_type="right_nero_box_pick_ee", use_videos=True,
...
def choose_failure(keypad=None) -> str:
    """选失败类型。序号选预设; 也可直接敲任意中英文当自定义原因记录。

    ⚠ 必须先把终端从 cbreak 恢复成行模式, 否则 KeyPad 关了回显 —— 你打的字看不见、
    也退不了格。读完再切回 cbreak, 不影响后续单键操作。
    """
    print("\n  失败类型:", flush=True)
    for i, (code, zh) in enumerate(FAILURE_TYPES, 1):
        print(f"    {i}. {code:<18} {zh}", flush=True)
    print(f"  可输入: 序号(1~{len(FAILURE_TYPES)}) / 任意文字作自定义原因(中英文均可) / 回车=other",
          flush=True)
...
def save_episode(ds, args, success: bool, failure_type: str, ik_stats: dict,
    ...
    with open(ann / "episode_outcomes.jsonl", "a") as f:
        f.write(json.dumps({
            ...
            "exec_arm": "right", "source": "rollout_ee",
...
def safe_enable(arm, args):
    """右臂使能防冲: 以当前实读关节角为伺服目标使能, 1.5 秒内监控漂移。"""
    arm.refresh_feedback()
    time.sleep(0.3)
    q0 = arm.read_joints()
    print("\n使能前关节角(新鲜):", np.round(q0, 3).tolist(), flush=True)
    print(f"⚠ 使能右臂(speed={args.speed_percent}%)。确认工作区无人手、急停在手边。", flush=True)
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Nero pi0.5 rollout 采集 —— 右臂抓盒装箱(末端位姿版, 2 子任务)")
    ap.add_argument("--host", default="<POLICY_SERVER_IP>")
    ap.add_argument("--port", type=int, default=8031,
                    help="rightbox EE serve 端口(GPU0; 原 v3d 检垃圾服务已停, 让位给它)")
    ap.add_argument("--root", default=None)
    ap.add_argument("--repo-id", default="local/nero_rightbox_ee_rollout")
    ...
    if args.root is None:
        ...
        args.root = NERO_ROOT / "lerobot_data" / "Rlinf" / "data" / "wholeprocess" / run_id / "e0_rightbox"
        args.repo_id = f"local/wholeprocess_{run_id}_e0_rightbox"
    ...
    if args.right_can in (None, "auto"):
        got = C.resolve_can_by_serial(C.CAN_SERIALS["right"])
        if got is None:
            raise RuntimeError(f"右臂 CAN 未找到(serial={C.CAN_SERIALS['right']})。"
                               f"检查臂上电/CAN 是否 up; 或 --right-can canX 手动指定。")
        args.right_can = got
    print(f"  右臂 CAN → {args.right_can}", flush=True)
    # 本任务只用 third + 右腕两路(config 的 repack 就这两个), 左腕不开
    for cam, attr in (("third_view", "third_camera"), ("right_wrist", "right_wrist_camera")):
    ...
    if getattr(args, "auto_switch", False) and not args.mock_cameras:
        got = C.resolve_video_by_serial(C.CAM_SERIALS["left_wrist"])
        if got is None:
            raise RuntimeError(
                f"--auto-switch 需要 left_wrist 相机(serial={C.CAM_SERIALS['left_wrist']}), 但没找到。"
                f" 插上它, 或去掉 --auto-switch 用手动空格。")
        args.left_wrist_camera = got
        print(f"  相机  left_wrist → {got}   {'(仅供自动切换)'}", flush=True)
    else:
        args.left_wrist_camera = None
    return args


# ══════════════════════ 主循环 ══════════════════════
# 本段结束后的过渡轨迹名(trajectories/<名字>.json)。
# 对应 E0 右臂抓盒 → E1 左臂抓盒。录了同名轨迹就优先回放它, 没录则回退 goto_home.py 直线插值归位。
# ⚠ 这个常量之前漏了定义 —— goto_home_inline() 一调就 NameError, 整段过渡直接崩。
DEFAULT_TRAJ = "transition_e0_to_e1"
...
def main() -> None:  # noqa: PLR0915, PLR0912
    ...
    print(f"\n任务: 右臂抓盒装箱【末端位姿版】 {len(SUBTASKS)} 子任务  模式={mode}", flush=True)
    ...
    arm = MockArm() if args.mock_robot else RightArm(args)
        ...
        with KeyPad() as keypad:
            ...
            while True:
                ...
                if label == "g":
                    ...
                    success = (label == "s")
                    ftype = "none" if success else choose_failure(keypad)
