...
#   stage56: pi05_nero_stage56_flap_closing_ee_v2  / stage56_ee_run1 / 14999 / 8029
# 端口错开 → 两个服务可以同时挂着, 方便对比。
#
#   GPU=0 PORT=8029 STEP=14999 bash serve_hezi_stage56_ee.sh
# 停: kill $(cat <LOG_DIR>/serve_stage56_flap_ee.pid)
set -uo pipefail
FORK=<OPENPI_REPO>          # ★ fork, 不是主仓
REPO=<OPENPI_REPO>        # 借它的 scripts/ 与 client 包
PY=<OPENPI_VENV>/bin/python
CFG=pi05_nero_stage56_flap_closing_ee_v2
CKPT_ROOT=<CKPT_ROOT_EE>/pi05_nero_stage56_flap_closing_ee_v2/stage56_ee_run1
GPU="${GPU:-0}"; PORT="${PORT:-8030}"; STEP="${STEP:-19999}"   # 100集版; 训练中时可用 STEP=10000
DIR="$CKPT_ROOT/$STEP"
mkdir -p <LOG_DIR>
LOG=<LOG_DIR>/serve_stage56_flap_ee_${STEP}_$(date +%Y%m%d_%H%M%S).log
...
setsid nohup "$PY" scripts/serve_policy.py --port "$PORT" \
  ...
  --policy.dir="$DIR" \
  > "$LOG" 2>&1 &
echo $! > <LOG_DIR>/serve_stage56_flap_ee.pid
echo "$LOG" > <LOG_DIR>/serve_stage56_flap_ee.logpath
echo "pid=$(cat <LOG_DIR>/serve_stage56_flap_ee.pid)"
echo
