#!/usr/bin/env bash
# Nero 双臂 —— 细化封盖(stage3/4 的 7 个子任务)LeRobot v2.1 采集入口。
# 关键: CAN 按【序列号】解析(插拔改 can 号也不会左右反), 相机 左2/右4/三6。
#
# 用法:
#   ./collect_box_packing_closing_refined.sh
#   DRAG=1 ./collect_box_packing_closing_refined.sh     # 让脚本进拖动示教(默认不进, 靠外部/Web 设)
#   LEFT_CAM=/dev/video2 RIGHT_CAM=/dev/video4 THIRD_CAM=/dev/video6 ./...  # 相机号变了就覆盖
#   ./collect_box_packing_closing_refined.sh --dry-run-state-machine       # 透传给采集脚本的参数
set -euo pipefail
export OPENCV_VIDEOIO_V4L_SELECTTIMEOUT=1

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
HEZI_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
CONFIG="${CONFIG:-${HEZI_ROOT}/configs/box_packing_closing_refined_task.yaml}"

# ---- 左右臂 CAN 适配器序列号(固定不变, 用它解析当前 canX) ----
LEFT_CAN_SERIAL="<LEFT_CAN_SERIAL>"   # 左臂(接网线那只)
RIGHT_CAN_SERIAL="<RIGHT_CAN_SERIAL>"  # 右臂

# ---- 相机: 你要求的映射 左2 / 右4 / 三6(与 config 一致; 改了插口就用 env 覆盖) ----
...
THIRD_RGB_EXPOSURE="${THIRD_RGB_EXPOSURE:-160}"; THIRD_RGB_GAIN="${THIRD_RGB_GAIN:-8}"; THIRD_RGB_GAMMA="${THIRD_RGB_GAMMA:-120}"
...
MOCK=0
for a in "$@"; do
  [[ "$a" == "--mock" || "$a" == "--mock-cameras" || "$a" == "--dry-run-state-machine" || "$a" == "--check-depth-only" ]] && MOCK=1
done
...
exec python3 "${SCRIPT_DIR}/collect_box_packing_closing_refined.py" \
  --config "$CONFIG" \
