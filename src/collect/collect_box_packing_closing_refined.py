#!/usr/bin/env python3
"""Refined Nero carton-closing collection for LeRobot v2.1.

This is a copy of collect_box_packing_long_episode.py narrowed to the box
closing part of the task. It records only refined closing/rotation subtasks;
the original long-horizon collector is unchanged.
"""
...
DEFAULT_CONFIG = HEZI_ROOT / "configs" / "box_packing_closing_refined_task.yaml"
...
def apply_repeat_overrides(cfg: LongTaskConfig, *, mouse_boxes: int | None, earphone_boxes: int | None) -> LongTaskConfig:
    ...
    if 1 not in cfg.stages or 2 not in cfg.stages:
        print("  [WARN] 当前是细化封盖采集配置，没有Stage 1/2；忽略 --mouse-boxes/--earphone-boxes。", flush=True)
        return cfg
...
class StageManager:
    ...
    def handle_space(self, frame_index: int, now_s: float | None = None) -> KeyResult:
        ...
        def advance(done_frame: int) -> None:
            ...
            if next_stage_ids:
                ...
                self.stage_completed = True
                print("  ✓ 最后一个细化封盖阶段已完成，进入WAIT_SAVE；按S成功保存，F失败保存，G丢弃。", flush=True)
    ...
    def status_lines(self, frame_count: int, elapsed_s: float, *, status: str) -> list[str]:
        ...
        return [
            ...
            f"Refined closing subtasks completed by Space: {len(self.segments)} / {self.planned_subtask_count}",
...
def success_key_sequence(cfg: LongTaskConfig) -> list[str]:
    # First SPACE starts the episode outside this helper; one SPACE completes each refined subtask.
...
def integrity_check(args: argparse.Namespace, cfg: LongTaskConfig, result: EpisodeResult, *, success: bool) -> None:
    ...
    if success:
        ...
        if len(result.segments) != expected_segments:
            raise RuntimeError(
                f"完整性检查失败: 成功episode应有{expected_segments}个细化子任务片段，实际 {len(result.segments)}"
            )
...
def wait_idle_start(
    ...
    *,
    ...
    print("请先把大小盒子放入箱内，把打包箱保持在待封盖状态，双臂回到统一初始姿态。")
    keys.drain()
    ep_cfg = cfg
    if ask_box_counts:
        if 1 not in cfg.stages or 2 not in cfg.stages:
    ...
    print(
        f"本条计划: 细化封盖子任务实例 {expected_segment_count(ep_cfg)} 个。"
    )
    ...
    workspace.write_info(
        {
            ...
            "collection_plan": "closing_refined",
        }
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=False,
        help="Compatibility option from the full box-packing collector; ignored by this refined closing-only collector.",
    )
...
def apply_config_defaults(args: argparse.Namespace, cfg: LongTaskConfig) -> argparse.Namespace:
    ...
    if args.dry_run_save_episode:
        ...
        if args.root is None:
            args.root = Path("/tmp/nero_box_packing_long_dryrun_episode")
        if args.repo_id is None:
            args.repo_id = "local/nero_box_packing_long_dryrun_episode"
        if args.fps is None:
            args.fps = 5
...
def print_startup(args: argparse.Namespace, cfg: LongTaskConfig) -> None:
    ...
    print(f"{len(cfg.prompts)}种细化prompt:")
    for idx, prompt in sorted(cfg.prompts.items()):
        print(f"  P{idx}: {prompt.zh}")
    print("按键: Space=开始/完成当前细化阶段并推进  S=成功保存  F=失败保存  G=丢弃  Q=退出")
    print("====================================================")
