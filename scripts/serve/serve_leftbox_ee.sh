...
#
#   GPU=0 PORT=8026 STEP=19999 bash serve_leftbox_ee.sh
# 停: kill $(cat <LOG_DIR>/serve_leftbox_ee.pid)
set -uo pipefail
FORK=<OPENPI_REPO>          # ★ fork, 不是主仓
REPO=<OPENPI_REPO>        # 借它的 scripts/ 与 client 包
PY=<OPENPI_VENV>/bin/python
CFG=pi05_nero_left_box_pick_ee_v1
CKPT_ROOT=<CKPT_ROOT_EE>/pi05_nero_left_box_pick_ee_v1/lbp_ee_run1
GPU="${GPU:-0}"; PORT="${PORT:-8032}"; STEP="${STEP:-19999}"
DIR="$CKPT_ROOT/$STEP"
mkdir -p <LOG_DIR>
LOG=<LOG_DIR>/serve_leftbox_ee_${STEP}_$(date +%Y%m%d_%H%M%S).log
...
setsid nohup "$PY" scripts/serve_policy.py --port "$PORT" \
  ...
  --policy.dir="$DIR" \
  > "$LOG" 2>&1 &
echo $! > <LOG_DIR>/serve_leftbox_ee.pid
echo "$LOG" > <LOG_DIR>/serve_leftbox_ee.logpath
echo "pid=$(cat <LOG_DIR>/serve_leftbox_ee.pid)"
echo
