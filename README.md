...
| 训练数据 | 400 集 / **268,034 帧** / 15Hz / 三路相机,全部自采 |
...
- 设计并训练了子任务进度头(SigLIP2 + MLP)做自动切换 —— 完整研究见
  **[subtask-progress-head](https://github.com/vfamatialas-ship-it/subtask-progress-head)**;本仓库这边补上了一个**最短时长下限**,
  原配置只有上限没有下限,一个本该 140 帧的子任务可能 14 帧就被切走
...
一个**子任务进度头**判断"当前这步做完了没有",做完就换喂给策略的那句 prompt:

```
三路相机 → SigLIP2-so400m(冻结) → MLP → (progress, done, time-to-boundary)
                                            ↓ 取 min = signal
   signal > τ 连续 K=8 帧  →  再等 D 步延迟补偿  →  换 prompt
```

核心约束是**只吃当前帧** —— 不喂帧号、不用时序模型,所以网络卡顿时输入不变、
进度天然不推进(实测注入 90 步卡顿,切换偏移 **0 帧**)。

所有阈值都是**步数量纲**、按 15Hz 标定,以 5Hz 调用会让准确率从 76% 崩到 4%,
因此切换器挂在**相机循环**上而非策略推理循环上,并做了流水线把 GPU 往返移出控制临界路径。

> **完整的方法、标定公式、消融、鲁棒性测试与负结果在单独的仓库:**
> **[subtask-progress-head](https://github.com/vfamatialas-ship-it/subtask-progress-head)**
> 包括零人工标签管线、22%→73.8% 的判据修正历程、3 种子噪声带、
> 四条无效改进路径,以及"专家级图像分类器不可行"这个负结果。
>
> 本仓库只保留把它接进 rollout 客户端的胶水层
> (`src/deploy/switcher_{client,service}.py`)。

---

## 数据集

400 集、268,034 帧、15Hz、三路 RGB —— 全部由拖动示教在真机上采集。
...
73 条留出 episode 串行回放(误差跨子任务累积,最接近真机行为):
**70/73 = 96%** 的 episode 走完全部子任务,切换时刻误差中位 **−1 帧**,
**76%** 落在 ±10 帧内、**91%** 落在 ±20 帧内(@15Hz,20 帧 = 1.33 秒)。

逐专家分解、消融与鲁棒性实验见 **[subtask-progress-head](https://github.com/vfamatialas-ship-it/subtask-progress-head)**。
...
│   ├── switcher_{client,service}.py   子任务进度头接入层(独立 venv + IPC)
│   │                                  └ 需 subtask-progress-head 仓库提供模型
